# From Giuseppe's script. It runs the analysis on a loci clump specified in the arguments
# For better reading, I removed the semicolon and separated the command with newlines
...
for (i in 1:n.loc) {
        ...
        if (!is.null(locus)) {
                ...
                loc.out = run.univ.bivar(locus, univ.thresh=1e-4) ### univ.thresh could be modified

		# store
                u[[i]] = cbind(loc.info, loc.out$univ)
		# if no phenotypes passed univ sig, out$bivar will be NULL, so check this before attempting to store bivar results as youll otherwise have an error
